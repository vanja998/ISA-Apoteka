...
$(document).on('click', '#btnCalendarWeek', function () {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:8081/counselings/getCounselingsWeek",
        dataType: "json",
...
$(document).on('click', '#btnCalendarMonth', function () {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:8081/counselings/getCounselingsMonth",
        dataType: "json",
        beforeSend: function(xhr) {
            if (localStorage.token) {
                xhr.setRequestHeader('Authorization', 'Bearer ' + localStorage.token);
...
$(document).on('click', '#btnCalendarYear', function () {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:8081/counselings/getCounselingsYear",
        dataType: "json",
        beforeSend: function(xhr) {
            if (localStorage.token) {
                xhr.setRequestHeader('Authorization', 'Bearer ' + localStorage.token);
